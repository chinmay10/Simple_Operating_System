...
void fun2() {
    ...
    Console::puts("Writing in block 1 initially"); Console::puts("\n");
    unsigned char temp_buf[8]={'*','b','u','f','f','e','r','*'};
    SYSTEM_DISK->write(1,temp_buf);
    ...
    for(int j = 0;; j++) {
       ...
       /* -- Read */
       Console::puts("Calling Read operation from disk for read block"); Console::puti(read_block); Console::puts("\n");
	...
	Console::puts("\n");
       ...
       Console::puts("Calling write operation on disk for block"); Console::puti(write_block); Console::puts("\n");
